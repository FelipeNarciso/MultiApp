...
import { validacaoTranslatedText } from '../utilities/validationLanguageTranslator' // Importa o arquivo com o esquema de validação
...
const LanguageTranslator = () => {
  ...
  const [erro, setErro] = useState('') // Define um estado para a tratativa de erro
  
  // Função para traduzir o texto
  const translateText = async () => {
    setErro('') // Limpa o erro toda vez que traduzir uma nova frase
    setTranslatedText('') // Limpa a frase traduzida anteriormente, para melhor identificação caso tiver um erro

    if(text == ''){
      setErro('Erro ao traduzir, o campo esta vazio!') // Exibe um erro ao usuário se tentar fazer a requisição com o campo vazio
      return
    }
    ...
    try {
      ...
      try{
        await validacaoTranslatedText.validate(response.data.responseData) // Faz a validação dos dados recebidos
        setTranslatedText(response.data.responseData.translatedText); // Armazena o texto traduzido no estado translatedText
      }catch(erroValidacao){
        setErro('Erro ao validar a API. Tente novamente!') // Exibe um erro se os dados recebidos não forem validos
      }
      
    } catch (error) {
      setErro('Erro ao requisitar a API. Tente novamente!'); // Exibe o erro para o usuário em caso de falha
    }
  };
