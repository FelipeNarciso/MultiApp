...
const IPAddressFinder = () => {
  ...
  const [erro, setErro] = useState('') // Define um estado para mostrar um erro se a requisição não funcionar 

  // Função para buscar os dados do IP
  const findIP = async () => {
    try {
      setIpData('') // Limpa o IP pesquisado antes para aparecer melhor o erro caso tenha
  ...
  return (
    <Container>
      ...
      <Error>{erro}</Error> {/* Mostra um erro para o usuário se a requisição não funcionar */}
